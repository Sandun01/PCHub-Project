...
class OrderServices {
  ...
  addItemToCart_Local(item) {
    ...
    if (data) {
      ...
      if (itemsArr === null) {
        ...
        //add new item to existing local
        ...
        if (itemExists === false) {
          ...
          //ADD ITEM TO LOCAL
          var resAddToArray = LocalStorageService.setItem(
            this.LOCAL_KEY,
            itemsArr
          );
  ...
  async addItemToCart_DB(item, userID) {
    ...
    await axios
      .put(BackendApi_URL + "/orders/addItem/user/" + userID, cartItem)
      .then((res) => {
  ...
  async getOrderByUserID(userID) {
    ...
    await axios
      .post(BackendApi_URL + "/orders/user/" + userID, data)
      .then((res) => {
        result = res;
      })
      .catch((error) => {
  ...
  async removeOrderItemByID(ordId, itmId) {
    ...
    await axios
      .put(BackendApi_URL + "/orders/removeItem/" + ordId, data)
      .then((res) => {
        result = res;
      })
      .catch((error) => {
  ...
  async editOrderItemQty(ordId, data) {
    ...
    await axios
      .put(BackendApi_URL + "/orders/qty/edit/" + ordId, data)
      .then((res) => {
        result = res;
      })
      .catch((error) => {
  ...
  async updatePaymentDetails(id, data) {
    var result = null;
    // console.log(data, id)
    result = await await axios.put(
      BackendApi_URL + "/orders/payment/" + id,
      data
    );
  ...
  async checkoutOrder(data, id) {
    var result = null;
    // console.log(data, id)
    result = await await axios.post(
      BackendApi_URL + "/orders/checkout/" + id,
      data
    );
  ...
  async changePaymentMethod(id, data) {
    ...
    // console.log(data, id)
    result = await await axios.put(
      BackendApi_URL + "/orders/payment/method/" + id,
      data
    );
  ...
  async generateQuotation(data) {
    ...
    try {
      var genPdfResult = await axios.post(
        BackendApi_URL + "/orders/generateQuotation",
        data
      );
      ...
      if (!Utils.isEmptyObject(genPdfResult)) {
        if (genPdfResult.status === 200) {
          ...
          //get bill pdf
          var genPdfResult2 = await axios.get(
            BackendApi_URL + "/orders/getQuotation",
            { responseType: "blob" }
          );
          ...
          if (!Utils.isEmptyObject(genPdfResult2)) {
            ...
            if (genPdfResult2.status === 200) {
              ...
              saveAs(pdfBlob, "newQuotation.pdf");
  ...
  async updateDeliveryDetails(data) {
    ...
    var result = await axios.put(
      BackendApi_URL + "/orders/delivery/edit/" + id,
      data
    );
    return result;
  }

  //Database/Backend -- generate report pdf
  async generateOrderReport(data){

    var result = 0;

    try{
      var genPdfResult = await axios.post(BackendApi_URL+"/orders/generateOrder_report", data);
      ...
      if(!Utils.isEmptyObject(genPdfResult)){
        ...
        if(genPdfResult.status === 200){ //pdf generate bill success
          ...
          //get bill pdf
          var genPdfResult2 = await axios.get(BackendApi_URL+"/orders/getOrder_report", { responseType: 'blob' });
          ...
          if(!Utils.isEmptyObject(genPdfResult2)){ //get bill pdf response success
            ...
            if(genPdfResult2.status === 200){
              ...
              saveAs(pdfBlob, 'newOrderReport.pdf');
      
              // console.log(genPdfResult2);
              result = 1;

            }
            else{ //get order bill pdf response not success
              result = 0;
            }
        ...
        else{ //generate order bill report response not success
          result = 0;
        }
