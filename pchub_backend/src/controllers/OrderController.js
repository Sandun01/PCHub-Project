...
async function checkOrderItemQuantities(id, itemsArr) {
  ...
  try {
    ...
    for (var i = 0; i < lenItems; i++) {
      ...
      var qtyData = await Product.findById(findProductQuery, {
        countInStock: 1,
        _id: 0,
      });
      // console.log(qtyData)
      ...
      if (isQtyDataEmpty == false) {
        ...
        if (cartQty > qtyDB) {
          //if quantity in cart exceeded the count in stock
          allInStock = 0;
...
const updatePaymentDetails = async (req, res) => {
  ...
  if (!isDataEmpty) {
    ...
    if (haveOutOfStockItems == 0) {
      ...
      res
        .status(200)
        .send({ success: false, message: 'Error in checking ordered items' });
    } else {
      var data = req.body;
      var paidStatus = data.isPaid;
      var paypalDetails = data.detailsByPaypal;
      ...
      if (qtyUpdated == -1) {
        res
          ...
          .send({
            success: false,
            message: 'Error in updating product quantities',
          });
      }
...
const generateFinalOrderBill = async (req, res) => {
  ...
  //generate bill
  pdf
    .create(finalBillTemplate(data), {})
    .toFile(`${__dirname}/files/OrderBill.pdf`, (err) => {
      if (err) {
...
const generateQuotation = async (req, res) => {
  ...
  //generate bill
  pdf
    .create(Quotation_Template(data), {})
    .toFile(`${__dirname}/files/Quotation.pdf`, (err) => {
      if (err) {
        res.send(Promise.reject());
      }
